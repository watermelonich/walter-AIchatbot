...
def compute_qkv(embeddings, phrases):
    Q = embeddings
    K = embeddings
    V = embeddings
    
    # Compute sentiment scores
    sentiment_scores = [analyzer.polarity_scores(phrase)["compound"] for phrase in phrases]
    sentiment_scores = np.array(sentiment_scores).reshape(-1, 1)
    
    # Repeat the sentiment scores for each embedding in the sequence
    sentiment_scores = np.repeat(sentiment_scores, embeddings.shape[0] // len(phrases), axis=0)
    
    # Concatenate sentiment scores to the embeddings
    embeddings = np.hstack((embeddings, sentiment_scores))
    
    return Q, K, V, embeddings
...
Q, K, V, embeddings = compute_qkv(embeddings, phrases)
output = self_attention(Q, K, V)
